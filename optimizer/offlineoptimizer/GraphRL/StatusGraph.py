...
class StatusGraph:
    def __init__(self, net, mc, Esafe, delta=100, T=20000):
        ...
        self.epi = 1
        self.q_table = defaultdict(lambda: np.zeros(len(self.net.listNodes))) # khoi tao bang Q
        self.list = []
    ...
    def dfs(self, vertex):
        if vertex == self.start:
            if random.random() < self.epi:
                vertex = self.list[random.randint(0, len(self.list) - 1)]
                self.path.append(vertex)
                self.visited[vertex.node.id] = True
            
        current_state = '-'.join([str(elem.node.id) for elem in self.path]) # trang thai hien tai
        ...
        for ver in vertex.adjacent:
            if not self.visited[ver.node.id]:
                ...
                if next is None:
                    next = ver
                    continue
                if self.q_table[current_state][ver.node.id] >= self.q_table[current_state][next.node.id]:
                    next = ver
        if next is None:
            return
        if random.random() < self.epi:
        ...
        # action tốt nhất của trạng thái tiếp theo
        next_state = '-'.join([str(elem.node.id) for elem in self.path])
        ...
        if vertex.node is not None:
            td_target = next.reward + discount_factor * self.q_table[next_state][best_next_action]
            td_delta = td_target - self.q_table[current_state][vertex.node.id]
            self.q_table[current_state][vertex.node.id] += alpha * td_delta
